...
class TimeAnalysisProcessor:
    ...
    def plays_by_day_of_week(self) -> list[int]:
        ...
        day_counts = Counter(track.played_at.weekday() for track in self.history.tracks)
    ...
    def peak_day_overall(self) -> dict:
        ...
        date_counts = Counter(track.played_at.date() for track in self.history.tracks)
        if not date_counts:
    ...
    def late_night_anthem(self) -> dict | None:
        ...
        late_night_tracks = [
            track for track in self.history.tracks if 0 <= track.played_at.hour < 4
        ]

        if not late_night_tracks:
            return None

        track_counts = Counter((track.title, track.artist) for track in late_night_tracks)
    ...
    def day_anthem(self, day_of_week: int) -> dict | None:
        ...
        day_tracks = [
            track for track in self.history.tracks if track.played_at.weekday() == day_of_week
        ]
